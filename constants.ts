import { EducationLevel, LocalizedString, Subject } from './types';
...
const mkSub = (id: string, ar: string, fr: string, en: string, coeff: number): Subject => ({
  id,
  name: { ar, fr, en },
  coefficient: coeff,
});
...
// Common Subjects
const ARABIC = (c: number) => mkSub('arab', 'اللغة العربية', 'Langue Arabe', 'Arabic', c);
const MATH = (c: number) => mkSub('math', 'الرياضيات', 'Mathématiques', 'Mathematics', c);
const PHYSICS = (c: number) => mkSub('phys', 'العلوم الفيزيائية', 'Physique', 'Physics', c);
const SCIENCE = (c: number) => mkSub('scien', 'علوم الطبيعة والحياة', 'Sciences Naturelles', 'Natural Sciences', c);
const FRENCH = (c: number) => mkSub('fren', 'الفرنسية', 'Français', 'French', c);
const ENGLISH = (c: number) => mkSub('eng', 'الإنجليزية', 'Anglais', 'English', c);
const ISLAMIC = (c: number) => mkSub('islam', 'العلوم الإسلامية', 'Sciences Islamiques', 'Islamic Sciences', c);
const HISTORY = (c: number) => mkSub('hist', 'التاريخ والجغرافيا', 'Histoire-Géo', 'History-Geo', c);
const CIVICS = (c: number) => mkSub('civ', 'التربية المدنية', 'Éducation Civique', 'Civics', c);
const SPORT = (c: number) => mkSub('sport', 'التربية البدنية', 'Éducation Physique', 'Physical Education', c);
const PHILO = (c: number) => mkSub('philo', 'الفلسفة', 'Philosophie', 'Philosophy', c);
const TECH_MECH = (c: number) => mkSub('tm', 'هندسة ميكانيكية', 'Génie Mécanique', 'Mechanical Eng.', c);
const TECH_GEN = (c: number) => mkSub('tech', 'تكنولوجيا (هندسة)', 'Génie (Méca/Elec/Civil)', 'Technology (Eng)', c);
const ECON = (c: number) => mkSub('eco', 'تسيير واقتصاد', 'Management', 'Management & Economy', c);
const LAW = (c: number) => mkSub('law', 'القانون', 'Droit', 'Law', c);
const AMAZIGH = (c: number) => mkSub('amz', 'الأمازيغية', 'Tamazight', 'Tamazight', c);
const ART = (c: number) => mkSub('art', 'تربية فنية/موسيقية', 'Art/Musique', 'Art/Music', c);
const COMP = (c: number) => mkSub('inf', 'إعلام آلي', 'Informatique', 'Computer Science', c);

// Define Education System Structure
export const EDUCATION_SYSTEM: EducationLevel[] = [
  // --- CEM (Middle School) ---
  {
    id: 'cem_1',
