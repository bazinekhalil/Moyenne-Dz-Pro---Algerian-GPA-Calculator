...
import { EDUCATION_SYSTEM, TRANSLATIONS } from './constants';
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 animate-fade-in">
      {EDUCATION_SYSTEM.map((l) => (
        ...
        >
          ...
          {isRTL ? <ChevronLeft className="text-gray-400 group-hover:text-emerald-500" /> : <ChevronRight className="text-gray-400 group-hover:text-emerald-500" />}
        </button>
      ))}
    </div>
  );
      ...
      <div className="mt-8 bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden animate-fade-in">
        <div className="p-4 bg-gray-50 border-b border-gray-200 flex justify-between items-center flex-wrap gap-2">
          <div className="flex items-center gap-2">
            ...
            {showSaveMsg && <span className="text-xs font-bold text-emerald-600 bg-emerald-100 px-2 py-1 rounded animate-pulse">{t.savedMsg[lang]}</span>}
          </div>
        ...
        <div className="p-4 bg-white border-t flex flex-col sm:flex-row gap-3 justify-end">
           <button 
             onClick={() => {
                ...
                // We keep saved data in localStorage but reset current view
             }}
